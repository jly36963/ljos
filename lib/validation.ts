...
import {Arguments, LjosInstance} from './ljos-factory.js';
import {DetailedArguments} from './typings/ljos-parser-types.js';

const specialKeys = ['$0', '--', '_'];

// validation-type-stuff, missing params, bad implications
...
export function validation(
  ...
  // Validate appropriate # of non-option arguments were provided, i.e., '_'.
  self.nonOptionCount = function nonOptionCount(argv) {
    ...
    const _s =
      positionalCount - ljos.getInternalMethods().getContext().commands.length;
  ...
  self.unknownArguments = function unknownArguments(
    ...
    Object.keys(argv).forEach(key => {
      if (
        ...
        !Object.prototype.hasOwnProperty.call(
          ljos.getInternalMethods().getParseContext(),
          key
        ) &&
    ...
    // https://github.com/ljos/ljos/issues/1861
    if (checkPositionals) {
      // Check for non-option args that are not in currentContext.commands
      // Take into account expected args from commands and ljos.demand(number)
      const demandedCommands = ljos.getInternalMethods().getDemandedCommands();
  ...
  let implied: Dictionary<string[]> = {};
  self.implies = function implies(key, values) {
...
export interface ValidationInstance {
  ...
  implies(key: string, values: string[]): void;
