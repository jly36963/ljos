...
import {assertNotStrictEqual, PlatformShim} from './typings/common-types.js';
import * as templates from './completion-templates.js';
import {isPromise} from './utils/is-promise.js';
import {parseCommand} from './parse-command.js';
import {UsageInstance} from './usage.js';
import {LjosInstance} from './ljos-factory.js';
import {Arguments, DetailedArguments} from './typings/ljos-parser-types.js';

// Add bash completions to your
// ljos-powered applications.
...
export class Completion implements CompletionInstance {
  completionKey = 'get-ljos-completions';
  ...
  constructor(
    private readonly ljos: LjosInstance,
  ...
  private defaultCompletion(
    ...
    for (let i = 0, ii = args.length; i < ii; ++i) {
      if (handlers[args[i]]) {
        ...
        const y = this.ljos.getInternalMethods().reset();
        builder(y, true);
        return y.argv;
      }
    }
  ...
  private commandCompletions(
    ...
    const parentCommands = this.ljos.getInternalMethods().getContext().commands;
    if (
      !current.match(/^-/) &&
  ...
  private optionCompletions(
    ...
    if (
      ...
      const options = this.ljos.getInternalMethods().getOptions();
      const positionalKeys =
        this.ljos.getInternalMethods().getGroups()[
          this.usage.getPositionalGroupName()
        ] || [];

      Object.keys(options.key).forEach(key => {
        const negable = options.boolean.includes(key);
  ...
  private choicesFromPositionalsCompletions(
    ...
    const offset = Math.max(
      ...
      this.ljos.getInternalMethods().getContext().commands.length +
    ...
    const choices =
      this.ljos.getInternalMethods().getOptions().choices[positionalKey] || [];
  ...
  private getPreviousArgChoices(args: string[]): string[] | void {
    ...
    const possibleAliases = [
      previousArgKey,
      ...(this.ljos.getInternalMethods().getAliases()[previousArgKey] || []),
    ];
  ...
  private completeOptionKey(
    ...
    if (!this.zshShell) {
      ...
      completions.push(
        dashes +
          `${key.replace(/:/g, '\\:')}:${desc.replace('__ljosString__:', '')}`
      );
    }
  }

  /** A custom completion function can be provided to completion() */
  ...
  /** Get a list of completion commans. ('args' is the string[] from the line to be completed) */
  getCompletion(args: string[], done: CompletionCallback): any {
    const current = args.length ? args[args.length - 1] : '';
    const argv = this.ljos.parse(args, true);
  ...
  /** Register a callback to perform custom completions (can be async) */
  registerFunction(fn: CompletionFunction) {
    this.customCompletionFunction = fn;
  }

  /** Set parsed aliases on ljos completion object */
...
export function completion(
  ...
  return new Completion(ljos, usage, command, shim);
}
