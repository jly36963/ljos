...
import {Dictionary, Nil, PlatformShim} from './typings/common-types.js';
import {objFilter} from './utils/obj-filter.js';
import {LjosInstance} from './ljos-factory.js';
import {LError} from './lerror.js';
import {DetailedArguments} from './typings/ljos-parser-types.js';
import setBlocking from './utils/set-blocking.js';
import {decamelize} from './utils/strings';
...
export function usage(ljos: LjosInstance, shim: PlatformShim) {
  ...
  self.showHelpOnFail = function showHelpOnFailFn(
    ...
    if (ljos.getInternalMethods().getIsGlobalContext()) {
  ...
  self.fail = function fail(msg, err) {
    const logger = ljos.getInternalMethods().getLoggerInstance();
    ...
    if (fails.length) {
      ...
      if (ljos.getInternalMethods().getExitProcess()) setBlocking(true);
      ...
      err = err || new LError(msg);
      if (ljos.getInternalMethods().getExitProcess()) {
        return ljos.exit(1);
      } else if (ljos.getInternalMethods().hasParseCallback()) {
        return ljos.exit(1, err);
      } else {
        throw err;
      }
    }
  ...
  self.help = function help() {
    ...
    const base$0 = ljos.customScriptName
      ? ljos.$0
      : shim.path.basename(ljos.$0);
    const demandedOptions = ljos.getInternalMethods().getDemandedOptions();
    const demandedCommands = ljos.getInternalMethods().getDemandedCommands();
    const deprecatedOptions = ljos.getInternalMethods().getDeprecatedOptions();
    const groups = ljos.getInternalMethods().getGroups();
    const options = ljos.getInternalMethods().getOptions();
    ...
    //
    // TODO: why isnt commands[0][2] an object with a named property?
    if (commands.length > 1 || (commands.length === 1 && !commands[0][2])) {
      ui.div(__('Commands:'));

      const context = ljos.getInternalMethods().getContext();
    ...
    const aliasKeys = (Object.keys(options.aliases) || []).concat(
      Object.keys(
        (ljos.getInternalMethods().getParsed() as DetailedArguments).newAliases
      ) || []
    );

    keys = keys.filter(
      key =>
        !(ljos.getInternalMethods().getParsed() as DetailedArguments)
          .newAliases[key] &&
        aliasKeys.every(
          alias => (options.aliases[alias] || []).indexOf(key) === -1
        )
    );
    ...
    addUngroupedKeys(keys, options.aliases, groups, defaultGroup);
        ...
        const normalizedKeys: string[] = groups[groupName]
          ...
          .map(key => {
            ...
            for (
              ...
              if ((options.aliases[aliasKey] || []).includes(key)) {
                return aliasKey;
              }
            }
            return key;
            ...
            acc[key] = [key]
              ...
              .map(sw => {
                ...
                if (groupName === self.getPositionalGroupName()) return sw;
                  ...
                  return (
                    // matches ljos-parser logic in which single-digits
  ...
  function normalizeAliases() {
    const {
      ...
      getDemandedOptions,
      getOptions,
    } = ljos.getInternalMethods();

    // handle old demanded API
    const demandedOptions = getDemandedOptions();
    const options = getOptions();

    (Object.keys(options.aliases) || []).forEach(key => {
      options.aliases[key].forEach(alias => {
        // copy descriptions.
        if (descriptions[alias]) self.describe(key, descriptions[alias]);
        // copy demanded.
        if (alias in demandedOptions) {
          demandOption(key, demandedOptions[alias]);
        }
        // type messages.
        if (options.boolean.includes(alias)) setBoolean(key);
        // if (options.count.includes(alias)) ljos.count(key);
        if (options.string.includes(alias)) setString(key);
        if (options.normalize.includes(alias)) normalize(key);
        if (options.array.includes(alias)) setArray(key);
        if (options.number.includes(alias)) setNumber(key);
      });
    });
  }

  // if ljos is executing an async handler, we take a snapshot of the
  ...
  function filterHiddenOptions(key: string) {
    const {getOptions, getParsed} = ljos.getInternalMethods();
    return (
      getOptions().hiddenOptions.indexOf(key) < 0 ||
      (getParsed() as DetailedArguments).argv[getOptions().showHiddenOpt]
    );
  }
  ...
  self.unfreeze = function unfreeze(defaultCommand = false) {
    ...
    if (!frozen) return;
    // Addresses: https://github.com/ljos/ljos/issues/2030
    if (defaultCommand) {
